# 20223124 이정윤 프로젝트 빌드 과정
처음 jekyll을 사용하기 위해 ruby를 깔고 시작하였다. 하지만 jekyll이 제대로 설치가 되지 않아 오류가 았다. 
이를 해결하기 위해 루비를 삭제하고  다른 버전을 설치하고 다시 jekyll을 설치하였다. 
jekjyll -v를 이용하여 지킬이 제대로 설치가 되었는지 확인하였고 새로운 디렉토리를 만들어 나만의 블로그 만들기를 시작하였다.

지킬의 기본테마로 먼저 블로그를 만들고 테마를 바꿔서 다시 시작하였지만. 바뀐테마로 깃허브에 푸쉬를 하여도 테마가 적용되지 않았다.
당시 문제점을 제대로 파악하지 못해서 다시 원점으로 돌아가서 시작하였다.
하지만 문제는 해결되지 않았고 같은 실수가 나타났다. 계속 고민하다가 기본 테마가 디렉토리에서 삭제가 안되어서 테마다 적용이 안된것으로 판단.
기본 테마를 지우려고 노력하였다. 그래서 처음부터 다시 원점으로 돌아가서 다시 시작하였다.

그러나 또 로컬 주소에서는 테마와 포스트가 다 적용되었지만 깃허브에 아무리 푸쉬를 해도 깃허브 주소로 들어가면 적용이 되지않았다.
일주일을 넘게 고민했지만 결국 해내지 못했다.
